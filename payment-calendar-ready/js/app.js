...
(function () {
  ...
  function renderTable() {
    const allNames = state.contragents.map((c) => c.name);
    const allDates = getPeriodDates();
    const today = todayStr();
    const cur = state.settingsDoc.currency || '';
    const payments = visiblePayments();

    const startStr = dateStr(allDates[0]);
    const endStr = dateStr(allDates[allDates.length - 1]);
    const periodPayments = payments.filter((p) => p.dueDate >= startStr && p.dueDate <= endStr);
    const namesWithData = new Set(periodPayments.map((p) => p.contragent));
    const datesWithData = new Set(periodPayments.map((p) => p.dueDate));

    const names = state.hideEmpty ? allNames.filter((n) => namesWithData.has(n)) : allNames;
    const periodDates = state.hideEmpty ? allDates.filter((d) => datesWithData.has(dateStr(d))) : allDates;
    ...
    if (names.length === 0 || periodDates.length === 0) {
      body.innerHTML = `<tr><td colspan="${names.length + 2}" class="empty-note">Нет данных за этот период — попробуйте отключить фильтр «Скрыть пустые» или выбрать другой период</td></tr>`;
      document.getElementById('table-foot-row').innerHTML = '';
      return;
    }
  ...
  function renderCellPaymentsList() {
    ...
    wrap.innerHTML = list.map((p) => {
      ...
      const noteLine = p.note ? `<div class="note-line">💬 ${esc(p.note)}</div>` : '';
      ...
      return `<div class="list-row list-row-stack">
        <div class="list-row-top">
          <div class="grow">
            <div class="name">${fmtMoney(p.amount)} ${esc(cur)}</div>
            <div class="meta">${p.paid ? 'Оплачено' : 'Не оплачено'}${overdueTag}</div>
          </div>
          ${btns}
        </div>
        ${noteLine}
  ...
  // ---------- калькулятор ----------
  let calc = { display: '0', prev: null, op: null, waitingNext: false };

  function calcRender() {
    document.getElementById('calc-display').textContent = calc.display;
  }
  function calcReset() {
    calc = { display: '0', prev: null, op: null, waitingNext: false };
    calcRender();
  }
  function calcInputDigit(d) {
    if (calc.waitingNext) { calc.display = d; calc.waitingNext = false; }
    else { calc.display = calc.display === '0' ? d : calc.display + d; }
    calcRender();
  }
  function calcInputDecimal() {
    if (calc.waitingNext) { calc.display = '0.'; calc.waitingNext = false; calcRender(); return; }
    if (!calc.display.includes('.')) { calc.display += '.'; calcRender(); }
  }
  ...
  function calcSetOperator(op) {
    ...
    if (calc.prev !== null && calc.op && !calc.waitingNext) {
      calc.prev = calcCompute(calc.prev, cur, calc.op);
      calc.display = String(roundClean(calc.prev));
    } else {
      calc.prev = cur;
    }
    calc.op = op;
    calc.waitingNext = true;
    calcRender();
  }
  function calcEquals() {
  ...
  function roundClean(n) { return Math.round(n * 100) / 100; }

  function openCalculator() {
    const popup = document.getElementById('calculator-popup');
    if (!popup.style.left) positionCalculatorDefault();
  ...
  function toggleCalculator() {
    const popup = document.getElementById('calculator-popup');
    if (popup.classList.contains('hidden')) { calcReset(); openCalculator(); }
    else closeCalculator();
  }
  function calcConfirmAndInsert() {
    calcEquals();
    document.getElementById('cell-add-amount').value = calc.display;
    closeCalculator();
  }

  function positionCalculatorDefault() {
    const popup = document.getElementById('calculator-popup');
    ...
    if (top + 320 > window.innerHeight - 10) top = Math.max(10, window.innerHeight - 330);
  ...
  function clampCalculatorPosition() {
    ...
    top = Math.min(Math.max(top, 4), window.innerHeight - rect.height - 4);
    popup.style.left = left + 'px';
    popup.style.top = top + 'px';
  }
  ...
  function makeCalculatorDraggable() {
    ...
    function dragMove(e) {
      ...
      left = Math.min(Math.max(left, 4), window.innerWidth - popup.offsetWidth - 4);
      top = Math.min(Math.max(top, 4), window.innerHeight - popup.offsetHeight - 4);
      popup.style.left = left + 'px';
      popup.style.top = top + 'px';
    }
    function dragEnd() {
  ...
  function bindCalculatorEvents() {
    if (bindCalculatorEvents._bound) return;
    bindCalculatorEvents._bound = true;
    document.getElementById('btn-calc-toggle').onclick = toggleCalculator;
    document.getElementById('btn-calc-close').onclick = closeCalculator;
    document.querySelectorAll('.calc-btn[data-digit]').forEach((btn) => {
      btn.onclick = () => calcInputDigit(btn.getAttribute('data-digit'));
    });
    document.querySelectorAll('.calc-btn[data-op]').forEach((btn) => {
      btn.onclick = () => calcSetOperator(btn.getAttribute('data-op'));
    });
    document.querySelector('[data-action="clear"]').onclick = calcReset;
    document.querySelector('[data-action="backspace"]').onclick = calcBackspace;
    document.querySelector('[data-action="sign"]').onclick = calcToggleSign;
    document.querySelector('[data-action="decimal"]').onclick = calcInputDecimal;
    document.querySelector('[data-action="equals"]').onclick = calcEquals;
    document.querySelector('[data-action="ok"]').onclick = calcConfirmAndInsert;

    makeCalculatorDraggable();
    window.addEventListener('resize', () => {
      if (!document.getElementById('calculator-popup').classList.contains('hidden')) clampCalculatorPosition();
    });

    // клавиатура и нампад — работают, когда калькулятор открыт и фокус не в текстовом поле
    document.addEventListener('keydown', (e) => {
      if (e.key === 'F4') {
        const modal = document.getElementById('modal-cell');
        if (modal.classList.contains('open') && !cellCtx.readonly) {
          e.preventDefault();
          toggleCalculator();
        }
        return;
      ...
      if (isTyping) return; // не мешаем обычному вводу в полях формы

      if (/^[0-9]$/.test(e.key)) { e.preventDefault(); calcInputDigit(e.key); }
      else if (e.key === '+') { e.preventDefault(); calcSetOperator('+'); }
      else if (e.key === '-') { e.preventDefault(); calcSetOperator('-'); }
      else if (e.key === '*') { e.preventDefault(); calcSetOperator('*'); }
      else if (e.key === '/') { e.preventDefault(); calcSetOperator('/'); }
      else if (e.key === '.' || e.key === ',') { e.preventDefault(); calcInputDecimal(); }
      else if (e.key === 'Backspace') { e.preventDefault(); calcBackspace(); }
      else if (e.key === 'Enter') { e.preventDefault(); calcConfirmAndInsert(); }
      else if (e.key === '=') { e.preventDefault(); calcEquals(); }
    });
  }
  ...
  function bindGlobalEvents() {
    ...
    // фильтр «скрыть пустые строки/столбцы»
    const hideEmptyCb = document.getElementById('filter-hide-empty');
    hideEmptyCb.checked = state.hideEmpty;
    hideEmptyCb.onchange = () => { state.hideEmpty = hideEmptyCb.checked; renderAll(); };
