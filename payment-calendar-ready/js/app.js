...
(function () {
  ...
  function showApp() {
    ...
    document.getElementById('toolbar-admin').style.display = isAdmin() ? 'flex' : 'none';
  ...
  function renderOverdueBanner() {
    ...
    inner.onclick = () => {
      ...
      buildPeriodSelectors(); renderAll();
    };
  ...
  function paymentsInMonth() {
    ...
    return state.payments.filter((p) => p.dueDate && p.dueDate.startsWith(prefix));
  }

  function renderSummary() {
    const cur = state.settingsDoc.currency || '';
    const monthPayments = paymentsInMonth();
    const total = monthPayments.reduce((s, p) => s + Number(p.amount || 0), 0);
    const paid = monthPayments.filter((p) => p.paid).reduce((s, p) => s + Number(p.amount || 0), 0);
  ...
  function renderTable() {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      const dateStr = `${state.year}-${pad(state.month + 1)}-${pad(day)}`;
      const dObj = new Date(state.year, state.month, day);
      ...
      names.forEach((name, ci) => {
        const cellPayments = state.payments.filter((p) => p.dueDate === dateStr && p.contragent === name);
        ...
        if (cellPayments.length === 0) {
          ...
          cellsHtml += `<td class="cell${editable}" data-date="${dateStr}" data-c="${esc(name)}"><span class="empty-dot">·</span></td>`;
        } else {
          ...
          cellsHtml += `<td class="cell ${statusClass}${rolledClass}${editable}" title="${esc(title)}" data-date="${dateStr}" data-c="${esc(name)}"><span class="amt">${fmtMoney(sum)}</span></td>`;
        }
      });
      ...
      rowsHtml += `<tr class="${rowClasses}">
        <td class="col-date">${pad(day)}.${pad(state.month + 1)} ${wd}</td>
        ${cellsHtml}
  ...
  function renderContragentsList() {
    ...
    wrap.innerHTML = state.contragents.map((c) => {
      ...
      return `<div class="list-row">
        ...
        <div class="row-btns"><button class="btn btn-sm btn-danger" data-id="${c.id}" data-name="${esc(c.name)}">Удалить</button></div>
      </div>`;
    }).join('');
  ...
  async function exportData() {
    const data = await Store.exportAll(state.contragents, state.payments, state.settingsDoc);
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = `payment-calendar-backup-${todayStr()}.json`;
  ...
  function importData(file) {
    ...
    reader.onload = async () => {
      try {
        ...
        if (!confirm('Данные из файла будут добавлены в общую базу (существующие записи не удаляются). Продолжить?')) return;
        await Store.importAll(data, state.contragents.map((c) => c.name));
        toast('Данные импортированы', 'ok');
      } catch (e) {
        toast('Не удалось прочитать файл: ' + authErrorMessage(e), 'err');
      }
    };
  ...
  function bindGlobalEvents() {
    ...
    if (isAdmin()) {
      document.getElementById('btn-contragents').onclick = () => { renderContragentsList(); openModal('modal-contragents'); };
      document.getElementById('btn-users').onclick = () => { renderUsersList(); openModal('modal-users'); };
      document.getElementById('btn-backup').onclick = () => openModal('modal-backup');
      ...
      document.getElementById('input-import').addEventListener('change', (e) => { if (e.target.files[0]) importData(e.target.files[0]); });
    }
  }
